...
class MSE_sDAC(torch.nn.Module):
    def __init__(self, normalization=True):
        super(MSE_sDAC, self).__init__()
        self.squared_difference = torch.nn.MSELoss(reduction='none')
        self.normalization = normalization
...
class Distortion_sDAC(torch.nn.Module):
    def __init__(self, args):
        super(Distortion_sDAC, self).__init__()
        if args.distortion_metric == 'MSE':
            self.dist = MSE_sDAC(normalization=False)
        elif args.distortion_metric == 'SSIM':
            self.dist = SSIM()
        elif args.distortion_metric == 'MS-SSIM':
            if args.trainset == 'CIFAR10':
